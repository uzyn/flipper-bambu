...
- The read path no longer re-detects the MIFARE Classic card type. The NFC app's
  ...
  given instead of calling `mf_classic_poller_sync_detect_type`. This drops the
  read from three full NFC poller cycles to one, removing the redundant heap
  churn and RF time that preceded every scan.
